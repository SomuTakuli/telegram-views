...
class Telegram:
        ...
        async with aiohttp.ClientSession(cookie_jar=jar, connector=connector) as session:
            try:
                async with session.get(
                    f'https://go.moneykamalo.com/yzpuELA', 
                    headers={
                        'referer': f'https://leher.to/imperialsmm/in/933DgW',
                    ...
                    if jar.filter_cookies(embed_response.url).get('stel_ssid'):
                        ...
                        if views_token:
                            views_response = await session.post(
                                ...
                                headers={
                                    'referer': f'https://go.moneykamalo.com/yzpuELA',
    ...
    def cli(self):
        logo = '''
        ~ Telegram Views Script By @legendx50 ~
          ~  ~
               ~  ~
        '''
...
parser.add_argument('-pt', '--post', dest='post', help='Post number', type=str, required=True)
